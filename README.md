...
A two-sided news platform: every story is built from multiple independent sources and shown with the two honest, named perspectives that actually divide people on it — never a single outlet's framing, never a forced left/right template. A community layer lets real readers add context, and enough independent readers converging on the same point can get folded back into the story itself.

Currently running as a live pilot for **Hebrew-speaking Israeli readers** — a deliberately narrow market chosen to test the "genuine multi-perspective" format before a possible return to English/international content. The pivot is a reversible config toggle (`NEXT_PUBLIC_LOCALE`), not a rewrite; the English mode and source list still exist underneath.
...
## Accounts, bookmarks, posts, and likes

All of this is real and server-backed via Supabase Auth + Postgres, not local-only: email/password sign-up (with an optional Turnstile CAPTCHA), bookmarks tied to your account, real reader posts (rate-limited, moderated by Claude), and persisted per-user likes.
