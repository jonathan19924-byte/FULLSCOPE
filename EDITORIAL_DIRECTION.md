...
| Heading weight/size (story titles) | `font-serif`, existing scale | Bump size/weight one step for story titles and section headers specifically | Skill's "News Editorial" (Newsreader/Roboto) pairing — same structural idea as the current Frank Ruhl Libre/Heebo pairing, just applied with more confidence |
...
**Constraint confirmed via the skill**: its boldest editorial typography option (Playfair Display + Source Serif 4 + JetBrains Mono, fully brutalist) has **no Hebrew glyph coverage** — not usable as-is since the app is Hebrew/RTL by default. Frank Ruhl Libre stays the right anchor if pushing the serif harder; it's one of the few dramatic serifs with genuine Hebrew support, and it's already loaded.
