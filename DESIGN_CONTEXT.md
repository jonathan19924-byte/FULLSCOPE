...
**Distribution**: the web app (Next.js, deployed on Vercel) is also shipped as a native iOS app via Capacitor — the native shell is a thin WebView pointed at the live production URL (no bundled/offline build), so almost every web change is live in the native app immediately with no App Store resubmission needed. Only genuinely native-code changes (a new Capacitor plugin, an Xcode capability/entitlement) require a new build and review. As of this writing version 1.2 (build 3) is live/approved — includes everything from 1.1 (push notifications, Sign in with Apple/Google, a camera fix, a new app icon) plus two push-notification reliability fixes shipped in 1.2 (device token registration, foreground push display). No version is currently pending.
...
- **Fonts**: locale-dependent, loaded via `next/font/google`, both pairs always bundled but only one applied per locale:
  ...
  - Hebrew/RTL (**currently active**): Heebo for both body and headings — Frank Ruhl Libre (a serif) was used for headings previously, but was dropped after readers found it hard to read; the font-loading code and CSS variable slot for a heading serif still exist, unused, so reintroducing one is a small change if wanted.
...
- **Community posts support threaded comments** (`community_post_comments`), rendered on each post's own detail page (`/posts/[id]`) rather than inline in the feed — the feed card (`post-feed-card.tsx`) shows just the count and links through.
...
- **A reader can write a post directly from the story they're reading**, not only from the standalone Create tab, via two entry points: a "Write a post" button in the "From readers" section (bottom of the page), and a persistent gold floating "+" button pinned above the tab bar for the whole time the reader is on the page — added because the inline button was easy to scroll past and never notice. Both open the same composer dialog with that story pre-tagged and locked; posting closes the dialog and stays on the page rather than navigating away. The standalone Create tab has no manual story-picker at all anymore (it used to be a plain `<select>` of every live story) — instead, on submit the post is auto-checked for a confident match against existing stories, and only if one is found does the reader see an inline confirm-to-link prompt; an unrelated post just posts immediately.
- **A floating action button is the established escape hatch from the fixed 5-item nav** (see the bottom-tab-bar bullet below) for a page-specific primary action that doesn't deserve its own nav destination — the story page's write-post FAB is the first and only instance so far. It sits at `bottom-20`-equivalent plus `env(safe-area-inset-bottom)` on mobile (matching the tab bar's own safe-area padding) so it doesn't get clipped by the home indicator on notched phones, and lower (`bottom-6`) on desktop where there's no tab bar to clear.
- **Once a post is linked to a story, a second Claude call classifies which of the two perspectives it leans toward** (`'A'`/`'B'`/untagged), run at submit time after the story link itself is settled — not on every keystroke. A confident result tags the post silently; a genuinely unclear one (neutral, both-sided, or off-topic) shows an inline confirm step using the two perspective titles as the choices, plus a "not sure / general" option, mirroring the existing confirm-to-link-a-story prompt directly above. An untagged post still posts normally — it's simply not forced into a side it doesn't fit, and doesn't get a slot in the perspective-tabbed reactions feed on the story page.
- **Individual posts have their own page** (`/posts/[id]`) rather than opening as an in-feed dialog — a real, shareable, server-rendered route with its own comments/like/bookmark/share/report/block, replacing the old dialog-on-`/posts` pattern. The header is a plain icon-only back button (`BackButton`, reused from Settings/Notifications) rather than a text link; a Follow button sits next to the author's name, shown only when the reader doesn't already follow them; the action row is comment count → like → bookmark → share, in that order; and the reply box is a single-line pill with a send-icon button rather than a stacked textarea + button.
- **Post bookmarks are a distinct save-for-later mechanic from story "likes."** A post's bookmark icon persists to its own `post_bookmarks` table — separate from the `bookmarks` table, which is keyed by story and already relabeled "Like" in the UI, to avoid conflating the two. The Bookmarks page (`/bookmarks`, and the Home page's embedded bookmarks tab) has a Stories/Posts tab toggle over the same underlying `BookmarksPageClient`.
- **Pull-to-refresh (`PullToRefresh`, a touch-gesture wrapper around `router.refresh()`) is applied to nearly every content page** — home, posts feed, post detail, story detail, profile (+ its follow-list sub-pages), bookmarks, notifications, search — but deliberately skipped on forms/auth pages (sign-in/up, create, settings), where a pull gesture reads as accidental-discard risk rather than a refresh affordance. Any new content-listing page should wrap its content in this by default; a new form/auth page should not.
- **A long "How this story developed" history collapses to the 5 most recent entries** with a "show N earlier updates" toggle, rather than always rendering the full list — a plain local `useState` toggle, not a shared/reusable Collapsible primitive (none exists in `src/components/ui/` yet; this was a single-use case, not worth the abstraction).
